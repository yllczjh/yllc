CREATE OR REPLACE PROCEDURE PR_病案首页_数据导出_医保中心(STR_参数          IN VARCHAR2,
...
BEGIN
  ...
  FOR ROW_手术记录 IN CUR_手术记录 LOOP
    ...
    STR_SQL := 'UPDATE 临时表_病案首页医保 SET SSBM' || ROW_手术记录.RN ||
  ...
  OPEN CUR_导出_列表信息 FOR
    SELECT T.* FROM 临时表_病案首页医保 T;
END PR_病案首页_数据导出_医保中心;
/
