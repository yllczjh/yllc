CREATE OR REPLACE PROCEDURE PR_病案首页西医_医保中心(STR_参数          IN VARCHAR2,
...
BEGIN
  ...
  FOR ROW_费用记录 IN CUR_费用记录 LOOP
    ...
    UPDATE 临时表_病案首页西医_医保中心 T
       SET T.ZFY      = ROW_费用记录.总金额,
           T.ZFJE     = ROW_费用记录.自付金额,
           T.ZFEJE    = ROW_费用记录.自付金额,
           T.QTZF    =
  ...
  FOR ROW_手术记录 IN CUR_手术记录 LOOP
    ...
    STR_SQL := 'UPDATE 临时表_病案首页西医_医保中心 SET SSBM' || ROW_手术记录.RN ||
  ...
  FOR ROW_诊断记录 IN CUR_诊断记录 LOOP
    ...
    IF ROW_诊断记录.诊断类型 = '出院诊断' THEN
      STR_SQL := 'UPDATE 临时表_病案首页西医_医保中心 SET ZYZD=:1,JBDM_S=:2,RYBQ=:3 WHERE JYH=:4';
      EXECUTE IMMEDIATE STR_SQL
        USING ROW_诊断记录.疾病名称, SUBSTR(ROW_诊断记录.ICD码, 0, 10), ROW_诊断记录.入院病情, ROW_诊断记录.住院病历号;
    ELSIF ROW_诊断记录.诊断类型 = '其他诊断' THEN
      STR_SQL := 'UPDATE 临时表_病案首页西医_医保中心 SET QTZD' || ROW_诊断记录.RN || '=:1,JBDM' ||
                 ROW_诊断记录.RN || '_S=:2,RYBQ' || ROW_诊断记录.RN ||
                 '=:3 WHERE JYH=:4';
      EXECUTE IMMEDIATE STR_SQL
        USING ROW_诊断记录.疾病名称, SUBSTR(ROW_诊断记录.ICD码, 0, 10), ROW_诊断记录.入院病情, ROW_诊断记录.住院病历号;
    ELSIF ROW_诊断记录.诊断类型 = '损伤和中毒外部原因' THEN
      STR_SQL := 'UPDATE 临时表_病案首页西医_医保中心 SET WBYY=:1,SSBM_S=:2 WHERE JYH=:3';
      EXECUTE IMMEDIATE STR_SQL
        USING ROW_诊断记录.疾病名称, SUBSTR(ROW_诊断记录.ICD码, 0, 10), ROW_诊断记录.住院病历号;
    ELSIF ROW_诊断记录.诊断类型 = '病理诊断' THEN
      STR_SQL := 'UPDATE 临时表_病案首页西医_医保中心 SET BLZD=:1,JBMM_S=:2 WHERE JYH=:3';
      EXECUTE IMMEDIATE STR_SQL
        USING ROW_诊断记录.疾病名称, SUBSTR(ROW_诊断记录.ICD码, 0, 10), ROW_诊断记录.住院病历号;
    END IF;
  END LOOP;
  ...
  OPEN CUR_导出_列表信息 FOR
    SELECT T.* FROM 临时表_病案首页西医_医保中心 T WHERE T.ZYZD!='无费退院';
END PR_病案首页西医_医保中心;
/
