...
begin
  ...
  begin
    if str_启用排班 = '是' then
      ...
      str_排班挂号类型编码 := FU_通用_截取字符串值(str_排班信息, '^', 3);
    
      str_排班记录ID := FU_通用_截取字符串值(str_排班信息, '^', 4);
      
      str_排班时段编码 := FU_通用_截取字符串值(str_排班信息, '^', 5);
      
      str_排班已挂号数 := FU_通用_截取字符串值(str_排班信息, '^', 6);
  ...
  begin
    ...
    if dat_出生日期<=to_date('1900-01-01','yyyy-MM-dd') then
     Str_返回信息 := '患者出生日期错误,非法出生日期,当前出生日期值为['||to_char(dat_出生日期,'yyyy-MM-dd')||']';
      GOTO 退出;
    end if;
    ...
    if int_病人信息行数 = 0 then
      ...
      update 基础项目_病人信息
         set 工作单位 = str_工作单位,手机号码=str_手机号码,固定电话=str_固定电话
       where 机构编码 = str_机构编码
         and 病人ID = str_病人ID;
