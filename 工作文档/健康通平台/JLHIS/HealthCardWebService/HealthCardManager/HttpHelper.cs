                    ...
                    else
                    {
                        switch (req_fun_code)
                        {
                            ...
                            case "1004":
                                ...
                                // 科室信息查询
                            case "2001":
                                GetDeptInfo(req_encrypted, ref res_code, ref res_msg, ref res_encrypted, ref res_sign);
                                break;
                            // 医生查询
                            case "2002":
                                GetDoctorInfo(req_encrypted, ref res_code, ref res_msg, ref res_encrypted, ref res_sign);
                                break;
                            // 排版信息查询
                            case "2003":
                                GetRegInfo(req_encrypted, ref res_code, ref res_msg, ref res_encrypted, ref res_sign);
                                break;
                                // 排班分时查询
                            case "2004":
...
;
        ...
        private static void GetDeptInfo(string req_encrypted, ref int res_code, ref string res_msg, ref string res_encrypted, ref string res_sign)
        {
            ...
            try
            {
                ...
                HealthCardDeptInfo_InParam deptInfo = new HealthCardDeptInfo_InParam();
                XmlHelper.AnalysisXmlReqDeptInfo(req_bxml, ref deptInfo);

                //设置返回业务参数
                 var   healthCardDeptInfo_Out = HisHelper.GetDeptInfo(deptInfo.DEPT_ID);
              
                if (healthCardDeptInfo_Out.DEPT_LIST.Count > 0)
                {
                    healthCardDeptInfo_Out.HOS_ID = HosID;
                    res_bxml = XmlHelper.AnalysisXmlResDeptInfo(healthCardDeptInfo_Out);
                    res_code = 0;
                    res_msg = "交易成功";
                    res_encrypted = AESHelper.EncryptForAES(res_bxml, key);
                    res_sign = Common.GetResponseSign(res_code, res_msg, res_encrypted, key);
