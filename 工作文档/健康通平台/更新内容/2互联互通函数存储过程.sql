...
prompt Created by syyyhl on 2020-07-09
set define off
spool 2互联互通函数存储过程.log
    ...
    BEGIN
      ...
      IF STR_病人ID IS NOT NULL THEN
        ...
        IF STR_患者姓名 <> STR_临时用户姓名 OR STR_患者性别 <> STR_临时用户性别 OR
           ...
           (STR_临时用户身份证号 <> STR_患者证件号码 AND STR_患者证件号码 IS NOT NULL AND
           STR_临时用户身份证号 IS NOT NULL) THEN
          INT_返回值   := 200711;
          STR_返回信息 := '就诊人身份信息和医院档案不匹配';
          GOTO 退出;
        END IF;
        ...
        BEGIN
          SELECT B.病人ID
           ...
           WHERE B.平台标识 = STR_平台标识
             AND B.医院编码 = STR_机构编码
             AND B.姓名 = STR_患者姓名
             AND NVL(B.证件号码, '空值') = (CASE --当为子女挂号时 证件类型有可能为空值
                   WHEN STR_患者证件号码 IS NULL THEN
                    '空值'
                   ELSE
                    STR_患者证件号码
                 END)
             AND B.监护人证件号码 = DECODE(STR_挂号类型, --当挂号类型为2时表示子女 需验证监护人身份证号码
                                    '2',
                                    STR_挂号人身份证号码,
                                    B.监护人证件号码)
             AND ROWNUM = 1;
        ...
        IF STR_病人ID IS NULL THEN
          ...
          BEGIN
            SELECT COUNT(1)
             ...
             WHERE T.平台标识 = STR_平台标识
               AND T.医院编码 = STR_机构编码
               AND T.证件号码 = STR_患者证件号码
               AND T.证件号码 IS NOT NULL
               AND T.姓名 <> STR_患者姓名;
          END;
          IF INT_返回值 > 0 THEN
            INT_返回值   := 200711;
          ...
          SELECT COUNT(1)
           ...
           WHERE B.机构编码 = STR_机构编码
             AND B.姓名 = STR_患者姓名
             AND NVL(B.身份证号, '空值') = (CASE --当为子女挂号时 证件类型有可能为空值
                   WHEN STR_患者证件号码 IS NULL THEN
                    '空值'
                   ELSE
                    STR_患者证件号码
                 END)
             AND C.监护人身份证号 = DECODE(STR_挂号类型, --当挂号类型为2时表示子女 需验证监护人身份证号码
                                    '2',
                                    STR_挂号人身份证号码,
                                    C.监护人身份证号);
        ELSE
        ...
        ELSE
          ...
          SELECT B.病人ID, B.婚姻状况, B.工作单位
            ...
            LEFT JOIN 基础项目_病人信息_其他 C
             ...
             AND B.病人ID = C.病人ID
           WHERE B.机构编码 = STR_机构编码
             AND ((STR_病人ID IS NULL AND B.姓名 = STR_患者姓名 AND
                 NVL(B.身份证号, '空值') = (CASE
                   WHEN STR_患者证件号码 IS NULL THEN
                    '空值'
                   ELSE
                    STR_患者证件号码
                 END) AND
                 C.监护人身份证号 = DECODE(STR_挂号类型, --当挂号类型为2时表示子女 需验证监护人身份证号码
                                      '2',
                                      STR_挂号人身份证号码,
                                      C.监护人身份证号)) OR
                 (STR_病人ID IS NOT NULL AND B.病人ID = STR_病人ID))
             AND ROWNUM = 1;
        END IF;
    ...
    BEGIN
      ...
      IF STR_调用标识 = '0' THEN
        STR_SQL      := 'SELECT ''' || STR_医院候诊号 ||
                        ''' AS HOSP_SERIAL_NUM,      
                         ''请到挂号窗口打印挂号单'' AS REMARK FROM DUAL';
        LOB_响应参数 := FU_互联互通_得到响应参数(STR_SQL, 'RES', '');
      ELSE
        LOB_响应参数 := STR_挂号单号;
      END IF;
